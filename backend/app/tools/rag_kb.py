...
def rag_kb_retriever(query: str, top_k: int = _DEFAULT_TOP_K) -> str:
    ...
    适用于：自由行攻略、路书、游记、目的地日程、景点与动线说明；以及特价机票/航班价格、航线价目表、航司或机场规则、政策说明、txt 及未识别为套餐表的 csv/xlsx 行文本等。
    用户询问「攻略」「自由行」「路书」「上传的文档/表」、或提到某省/某城玩法且需要引用站内上传材料时，应调用本工具（不要只用 travel_deals 的 vector_store_retriever）。
